...
import { MDepartmentService, MCompanyService, MApprovalDepartmentMenuService, MPositionService, MUserService, MMenuService, MAuthorityService } from '../../../services';
...
@Component({
	selector: 'app-system-approval-department-menu-list',
	templateUrl: './approval-department-menu-list.component.html',
	providers: [MDepartmentService, MCompanyService, MApprovalDepartmentMenuService, MPositionService, MUserService, MMenuService, MAuthorityService]
})
export class ApprovalDepartmentMenuListComponent implements OnInit {
	...
	constructor(private _Router: Router,
		...
		private _MApprovalDepartmentMenuService: MApprovalDepartmentMenuService,
		 ...
		 *==============================================*/
		if (this._param['m_menu_id']) this.Item['m_menu_id'] = this._param['m_menu_id'];
		if (this._param['menu_division_id']) this.Item['menu_division_id'] = this._param['menu_division_id'];
	...
	ngOnInit(){
		...
		this.getListUserRoute(this._param);
	 ...
	 * Get User Route
	 *==============================================*/
	getListUserRoute(params) {

		var objMenu = [];
		var select_m_menu_id = "";
		let paramData: URLSearchParams = new URLSearchParams();
		paramData.set('department_id', params['department_id']);
		paramData.set('business_id', params['business_id']);
		paramData.set('division_id', params['division_id']);
		paramData.set('enable_start_date', params['enable_start_date']);

		if(params['m_menu_id']){
			if(typeof params['m_menu_id'] == 'object' && params['m_menu_id'].length > 0){
				select_m_menu_id = params['m_menu_id'][0];
			}else{
				select_m_menu_id = params['m_menu_id'];
			}
		...
		this._MApprovalDepartmentMenuService.getListUserRoute(paramData).subscribe(res => {
	...
	onSelectFilterMenuChange(event, type?: string) {
		...
		if (type == 'deselected') {
			...
			if (this.Item['m_menu_id']) {
				if(this.Item['department_id'] != '' && this.Item['department_id'] != null){
					var filter_department_id = this.Item['department_id'];
				}else if(this.Item['division_id'] != '' && this.Item['division_id'] != null){
					var filter_department_id = this.Item['division_id'];
				}else if(this.Item['business_id'] != '' && this.Item['business_id'] != null){
					var filter_department_id = this.Item['business_id'];
				}

				if(filter_department_id){
					this.getEnableStartDate(filter_department_id, this.Item['m_menu_id']);
				}
			}
	 ...
	 *==============================================*/
	getEnableStartDate(department_id, m_menu_id) {
		var menu_type = '';
		if (m_menu_id) {
		...
		this._MApprovalDepartmentMenuService.getEnableStartDate(department_id, m_menu_id, menu_type).subscribe(res => {
	...
	onSearch() {
		...
		this.getListUserRoute(this.Item);

		// Change URL when submit
		this._Router.navigate(['/system/approval-department-menu'], { queryParams: this.Item });
	}
	...
	ngAfterViewInit(){
		...
		this.DTList = $('#tbl-data-approval-department-menu').DataTable({
			autoWidth: false,
	...
	protected savingData() {
		...
		paramData.set('department_id', this.Item['department_id']);
		paramData.set('menu_type', objMenu[0]);
		paramData.set('m_menu_id', objMenu[1]);
		...
		this._MApprovalDepartmentMenuService.save(paramData).subscribe(res => {
			$('.loading').hide();
			if (res.status == 'success') {
	...
	onDownload(form: NgForm) {
		if(form.valid){
			...
			this._MApprovalDepartmentMenuService.exportListApprovalDepartmentMenu(params).subscribe(res => {
				if (res.status == 'success') {
