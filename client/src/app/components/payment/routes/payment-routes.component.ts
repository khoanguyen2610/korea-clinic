...
import { TApprovalStatusService, TRequestService, MDepartmentService, MCompanyService, MApprovalMenuService, MPositionService,
		 MUserService, MAuthorityService, AuthService, ApprovalRoutesService
		} from '../../../services';
...
@Component({
	...
	providers: [ ApprovalRoutesService, TApprovalStatusService, TRequestService, MApprovalMenuService, MAuthorityService, MCompanyService, MDepartmentService, MPositionService, MUserService ]
})
...
export class PaymentRoutesComponent implements OnInit {
		...
		this._TRequestService.getByID(this._params.id).subscribe(res => {
			if(res.status=='success' && res.data){
				...
				if(+res.data.m_petition_status_id == 1 && !this.checkSessionCreatedForm(2, res.data.id)){
					...
					//get form approval list
					if(this.Item['routes'].length){
	...
	ngAfterViewInit() {
		...
		this.DTList = $('#tbl-data-payment-detail').DataTable({
			autoWidth: false,
	...
	getMasterRoutes(Item){
		...
		//set menu_type for form 0.4
		paramData.set('menu_type', 'payment');
		paramData.set('m_menu_id', Item['m_request_menu_id']);
	...
	onSearch() {
		...
		this.getListUserRoute(this.Item['m_request_menu_id']);
	...
	onBack(id) {
		if (this.param['permission']) {
			this._Router.navigate(['/payment/detail', id], { queryParams: { previous_page: this.param['previous_page'] } });
		} else {
			this._Router.navigate(['/payment/form/update', id]);
		}
	}
	...
	onSave() {
		...
		//set menu_type for form 0.4
		paramData.set('menu_type', 'payment');
		paramData.set('m_menu_id', this.Item['m_request_menu_id']);
		paramData.set('m_department_id', this.current_user_info['active_m_department_id']);
		paramData.set('user_check', '1');
		...
		this._TApprovalStatusService.setRoutes(paramData, this._params.id).subscribe(res => {
			if (res.status == 'success') {
				...
				if (this.param['permission'] == 1 && this.param['previous_page']) {
					this._Router.navigate(['/payment/detail', this._params['id']], { queryParams: { previous_page: this.param['previous_page'] } });
				} else {
					let current_path = this._Location.path();
					this._Router.navigate(['/payment/detail', this._params['id']], { queryParams: { previous_page: encodeURIComponent(current_path) } });
				}
