...
@Injectable()
export class TProposalService {
    private serviceUrl: string = 'tproposal/';
    ...
    public getNextAndPrevious(id: number) {
        let headers = new Headers();
        this.createAuthorizationHeader(headers);
        return this._Http.get(this._Configuration.apiUrl + this.serviceUrl + 'next_and_previous/' + id, {
            headers: headers,
    ...
    public upload(formData: FormData, id?: number): Promise<any> {
        return new Promise((resolve, reject) => {
            ...
            TProposalService.setUploadUpdateInterval(500);
    ...
    public uploadFilesAttachment(formData: FormData, id?: number): Promise<any> {
        return new Promise((resolve, reject) => {
            let xhr: XMLHttpRequest = new XMLHttpRequest();
            xhr.withCredentials = true;
            xhr.onreadystatechange = () => {
            ...
            xhr.open('POST', this._Configuration.apiUrl + this.serviceUrl + 'upload_files_attachment/' + id, true);
            xhr.setRequestHeader('Authorization', 'Basic ' + this._Configuration.apiAuth);
            xhr.withCredentials = true;
            xhr.send(formData);
        });
