...
fbUtils.fieldRender = function (fieldData, opts) {
  ...
  switch (fieldData.type) {
    ...
    case 'radio-group':
      ...
      if (fieldOptions) {
        ...
        for (var _i = 0; _i < fieldOptions.length; _i++) {
          ...
          optionsMarkup += '<label for="' + optionAttrs.id + '"><input ' + _optionAttrsString + ' />' + optionAttrs.label + '<div class="check"></div></label>';
        }
    ...
    case 'text':
      var unit_span = '';
      if(fieldData.unit) {
        unit_span = '<span class="has-unit-span" style="float: left">' + fieldData.unit + '</span>';
      }
      fieldMarkup = fieldLabel + ' <input style="float: left" ' + fieldDataString + '>' + unit_span;
    ...
    case 'autocomplete':
      ...
      break;
    case 'file':
      fieldMarkup = fieldLabel + ' <div class="uploader"><input ' + fieldDataString + '><span class="filename">選択ファイル無し</span><span class="btn-uploader btn">ファイル選択 </span></div>';
      break;
    case 'color':
  ...
  var FormBuilder = function FormBuilder(options, element) {
    ...
    var appendNewField = function appendNewField(values) {
      var type = values.type || 'text',
          ...
          delBtn = utils.markup('a', null, {
        id: 'del_' + lastID,
        className: 'del-button btn delete-confirm fa fa-trash',
      ...
      // Field preview Label
      liContents += '<label class="field-label">' + label;
      ...
      var field = utils.markup('li', liContents, {
        'class': type + '-field form-field clearfix',
        'type': type,
        id: lastID
      }),
          $li = $(field);
    ...
    formBuilder.actions = {
      ...
      exportData: _helpers.exportData,
      save: _helpers.save,
