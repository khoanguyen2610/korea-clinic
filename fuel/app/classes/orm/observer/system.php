...
class Observer_System extends Observer
{
    ...
    public function before_insert(\Orm\Model $obj){
        $obj->created_date = $this->_mysql_timestamp ? \Date::time()->format('mysql') : \Date::time()->get_timestamp();
        $obj->created_user_id = \Auth::get('id');
        $obj->updated_date = null;
        $obj->updated_user_id = null;
    }
    ...
    public function before_save(\Orm\Model $obj){
		...
		if(array_key_exists('deleted_date', $objProperties))
			unset($objProperties['deleted_date']);
		if(array_key_exists('deleted_date', $objProperties))
			unset($objProperties['deleted_date']);

		if ($obj->is_changed(array_keys($objProperties))){
	        $obj->updated_date = $this->_mysql_timestamp ? \Date::time()->format('mysql') : \Date::time()->get_timestamp();
	        $obj->updated_user_id = \Auth::get('id');
		}
    }
}