...
class Controller_News extends \Controller_API {
    ...
    public function get_detail($pk = null){
        ...
        $data = \Model_News::getDetail($pk, $param);
    ...
    public function post_index($pk = null){
        ...
        $validation->add_field('news_category_id',__('News Category', [], 'News Category'),'required');
        ...
        if($validation->run()){
			...
			!isset($arrData['date']) && $arrData['date'] = date('Y-m-d H:i:s');


            //======================== Default Data =================
			try{
				\DB::start_transaction();
	            $obj = empty($pk)?\Model_News::forge():\Model_News::find($pk);
                ...
                if($has_upload){
                    ...
                    foreach(\Upload::get_files() as $file) {
						...
						//Resize image
						\Vision_Common::resize_image(FILESPATH . NEWS_DIR . $today_dir . '/' . $file['saved_as']);
                        break;
                    }
					//Now just save first image
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
			try{
				\DB::start_transaction();
	            $result = \Model_News::softDelete($pk, array('item_status' => 'delete'));
				\DB::commit_transaction();
			} catch (\Exception $e) {
		      	\DB::rollback_transaction();
				/*==================================================
    ...
    public function delete_item_key($item_key = null){
        ...
        if(!empty($item_key)){
            try{
                ...
                $items = \Model_News::find('all', ['select' => ['id'], 'where' => ['item_key' => $item_key]]);

                if(!empty($items)){
                    foreach ($items as $val) {
                        $Ids[] = $val->id;
                        $result = \Model_News::softDelete($val->id, array('item_status' => 'delete'));
                    }
                }
                \DB::commit_transaction();
            } catch (\Exception $e) {
