...
class Controller_Slide extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'slide';
    ...
    public function get_list_data(){
		$param = \Input::param();
        $result     = \Model_Slide::listData($this->_arrParam['post_params'], array('task'=>'list-dbtable'));
        $items      = $result['data'];
    ...
    public function get_detail($pk = null){
        ...
        $data = \Model_Slide::getDetail($pk, $param);
    ...
    public function post_index($pk = null){
        ...
        if($validation->run()){
			...
			try{
	            ...
	            $obj = empty($pk)?\Model_Slide::forge():\Model_Slide::find($pk);
                ...
                if($has_upload){
                    ...
                    foreach(\Upload::get_files() as $file) {
						...
						//Resize image
						\Vision_Common::resize_image(FILESPATH . SLIDE_DIR . $today_dir . '/' . $file['saved_as']);
                        break;
                    }
 					//Now just save first image
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
			try{
				\DB::start_transaction();
	            $result = \Model_Slide::softDelete($pk, array('item_status' => 'delete'));
				\DB::commit_transaction();
			} catch (\Exception $e) {
		      	\DB::rollback_transaction();
				/*==================================================
