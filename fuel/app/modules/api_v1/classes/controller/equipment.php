...
class Controller_Equipment extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'equipment';
    ...
    public function get_detail($pk = null){
        ...
        $data = \Model_Equipment::getDetail($pk, $param);
    ...
    public function post_index($pk = null){
        ...
        if($validation->run()){
			...
			try{
	            ...
	            $obj = empty($pk)?\Model_Equipment::forge():\Model_Equipment::find($pk);
                ...
                if($has_upload){
                    ...
                    foreach(\Upload::get_files() as $file) {
						...
						//Resize image
						\Vision_Common::resize_image(FILESPATH . EQUIPMENT_DIR . $today_dir . '/' . $file['saved_as']);
                        break;
                    }
 					//Now just save first image
