 ...
 * @Last Modified time: 2017-03-09 09:39:24
 */

namespace Api_v1;
use \Controller\Exception;

class Controller_Master_MNoticeFee extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'm_notice_fee';
    ...
    public function post_index($pk = null){
        ...
        $validation->add_field('m_request_menu_id', __('Request Menu', [], 'Request Menu'), 'required');
        $validation->add_field('amount', __('Amount', [], 'Amount'), 'required');

        //Validate Unique Data
        if(!empty($pk)){
            $validation->field('m_request_menu_id', __('Request Menu', [], 'Request Menu'))->add_rule('unique', $this->main_table . '.id.m_request_menu_id.' . $pk);
        }else{
            $validation->field('m_request_menu_id', __('Request Menu', [], 'Request Menu'))->add_rule('unique', $this->main_table . '.id.m_request_menu_id');
        }
        ...
        if($validation->run()){
            ...
            //======================== Default Data =================
            $Item = empty($pk)?\Model_MNoticeFee::forge():\Model_MNoticeFee::find($pk);
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
            $result = \Model_MNoticeFee::softDelete($pk, array('item_status' => 'delete'));
            $status = 'success';
    ...
    public function get_list_data(){
        $result     = \Model_MNoticeFee::listData($this->_arrParam['post_params'], array('task'=>'list-dbtable'));
        $items      = $result['data'];
