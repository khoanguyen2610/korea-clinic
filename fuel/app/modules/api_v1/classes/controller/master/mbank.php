...
/**
 * @Author: k_nguyen
 * @Date:   2016-11-14 11:44:57
 * @Last Modified by:   k_nguyen
 * @Last Modified time: 2017-03-09 09:39:29
 */
namespace Api_v1;
use \Controller\Exception;

class Controller_Master_MBank extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'm_bank';
    ...
    public function post_index($pk = null){
        ...
        if(!empty($pk)){
            $validation->field('code',__('Code', [], 'Code'))->add_rule('unique', $this->main_table . '.id.code.'.$pk);
        }else{
            $validation->field('code',__('Code', [], 'Code'))->add_rule('unique', $this->main_table . '.id.code');
        }
        ...
        if($validation->run()){
            ...
            //======================== Default Data =================
            $Item = empty($pk)?\Model_MBank::forge():\Model_MBank::find($pk);
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
            $result = \Model_MBank::softDelete($pk, array('item_status' => 'delete'));
            $status = 'success';
    ...
    public function get_list_data(){
        $result     = \Model_MBank::listData($this->_arrParam['post_params'], array('task'=>'list-dbtable'));
        $items      = $result['data'];
