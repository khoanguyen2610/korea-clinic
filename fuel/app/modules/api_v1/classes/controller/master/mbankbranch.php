...
/**
 * @Author: k_nguyen
 * @Date:   2016-11-14 11:46:47
 * @Last Modified by:   k_nguyen
 * @Last Modified time: 2017-03-09 09:39:28
 */
namespace Api_v1;
use \Controller\Exception;

class Controller_Master_MBankBranch extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'm_bank_branch';
    ...
    public function post_index($pk = null){
        ...
        $validation->add_field('m_bank_id',__('Bank ID', [], 'Bank ID'),'required');
        ...
        if($validation->run()){
            ...
            //======================== Default Data =================
            $Item = empty($pk)?\Model_MBankBranch::forge():\Model_MBankBranch::find($pk);
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
            $result = \Model_MBankBranch::softDelete($pk, array('item_status' => 'delete'));
            $status = 'success';
    ...
    public function get_list_data(){
        $result     = \Model_MBankBranch::listData($this->_arrParam['post_params'], array('task'=>'list-dbtable'));
        $items      = $result['data'];
