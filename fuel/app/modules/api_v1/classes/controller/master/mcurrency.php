 ...
 * @Last Modified time: 2017-03-09 09:39:27
 */

namespace Api_v1;
use \Controller\Exception;

class Controller_Master_MCurrency extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'm_currency';
    ...
    public function post_index($pk = null){
        ...
        if($validation->run()){
            ...
            //======================== Default Data =================
            $Item = empty($pk)?\Model_MCurrency::forge():\Model_MCurrency::find($pk);
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
            $result = \Model_MCurrency::softDelete($pk, array('item_status' => 'delete'));
            $status = 'success';
    ...
    public function get_list_data(){
        $result     = \Model_MCurrency::listData($this->_arrParam['post_params'], array('task'=>'list-dbtable'));
        $items      = $result['data'];
