...
class Controller_Master_MAuthority extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'm_authority';
    ...
    public function post_index($pk = null){
        ...
        if($validation->run()){
            ...
            //======================== Default Data =================
            $Item = empty($pk)?\Model_MAuthority::forge():\Model_MAuthority::find($pk);
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
            $result = \Model_MAuthority::softDelete($pk, array('item_status' => 'delete'));
            $status = 'success';
     ...
     * Response data: status[success|error], message[notification]
     *=============================================================*/
    public function get_options_set_routes(){
        $param          = \Input::param();
        $data         = [];
         ...
         *=======================================*/
        $query = \DB::select('SM.id', 'SM.code', 'SM.name')
                    ->from(['m_authority', 'SM'])
                    ->where('SM.item_status', '!=', 'delete')
                    ->and_where('SM.id', 'IN', [2, 3, 4]);
