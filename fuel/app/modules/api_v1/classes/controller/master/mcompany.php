...
class Controller_Master_MCompany extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'm_company';
    ...
    public function post_index($pk = null){
        ...
        if($validation->run()){
            ...
            //======================== Default Data =================
            $Item = empty($pk)?\Model_MCompany::forge():\Model_MCompany::find($pk);
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
            $result = \Model_MCompany::softDelete($pk, array('item_status' => 'delete'));
            $status = 'success';
    ...
    public function get_list_data(){
        $result     = \Model_MCompany::listData($this->_arrParam['post_params'], array('task'=>'list-dbtable'));
        $items      = $result['data'];
