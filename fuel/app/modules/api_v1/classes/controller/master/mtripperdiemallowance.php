 ...
 * @Last Modified time: 2017-03-09 09:39:20
 */

namespace Api_v1;
use \Controller\Exception;

class Controller_Master_MTripPerdiemAllowance extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'm_trip_perdiem_allowance';
     ...
     * Method GET
     * Table m_trip_perdiem_allowance
     * Single data | array
    ...
    public function post_index($pk = null){
        ...
        $validation->add_field('m_trip_area_id',__('Trip Area ID', [], 'Trip Area ID'),'required');
        $validation->add_field('m_position_id',__('Position', [], 'Position'),'required');
        

        if($validation->run()){
            //Check exist
            if(!empty($pk)){
                $exist = \Model_MTripPerdiemAllowance::find('first', ['where' => [['id', '!=', $pk],
                                                                                    'm_trip_area_id' => $param['m_trip_area_id'], 
                                                                                    'm_position_id' => $param['m_position_id'],
                                                                                    'item_status' => 'active']]);
            }else{
                $exist = \Model_MTripPerdiemAllowance::find('first', ['where' => ['m_trip_area_id' => $param['m_trip_area_id'], 
                                                                                    'm_position_id' => $param['m_position_id'],
                                                                                    'item_status' => 'active']]);
            }
            if($exist){
                $validation_errors = ['exist' => 'この情報は既に存在しています。'];
                /*==================================================
                 * Response Data
                 *==================================================*/
            ...
            //======================== Default Data =================
            $Item = empty($pk)?\Model_MTripPerdiemAllowance::forge():\Model_MTripPerdiemAllowance::find($pk);
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
            $result = \Model_MTripPerdiemAllowance::softDelete($pk, array('item_status' => 'delete'));
            $status = 'success';
    ...
    public function get_list_data(){
        $result     = \Model_MTripPerdiemAllowance::listData($this->_arrParam['post_params'], array('task'=>'list-dbtable'));
        $items      = $result['data'];
