...
class Controller_Master_MTax extends \Controller_API {
	public function before() {
        parent::before();
        $this->main_table = 'm_tax';
    ...
    public function post_index($pk = null){
        ...
        $validation->add_field('value',__('Value', [], 'Value'),'required');
        ...
        if($validation->run()){
            ...
            //======================== Default Data =================
            $Item = empty($pk)?\Model_MTax::forge():\Model_MTax::find($pk);
    ...
    public function delete_index($pk = null){
        if(!empty($pk)){
            $result = \Model_MTax::softDelete($pk, array('item_status' => 'delete'));
            $status = 'success';
    ...
    public function get_list_data(){
        $result     = \Model_MTax::listData($this->_arrParam['post_params'], array('task'=>'list-dbtable'));
        $items      = $result['data'];
