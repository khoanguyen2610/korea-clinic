...
class Controller_Service extends \Controller_API {
    ...
    public function get_list_all(){
        $param = \Input::param();
        $data     = \Model_Service::getAll($param);
			...
			try{
                ...
                if($has_upload){
                    ...
                    foreach(\Upload::get_files() as $file) {
						...
						//Resize image
						\Vision_Common::resize_image(FILESPATH . SERVICE_DIR . $today_dir . '/' . $file['saved_as']);
                        break;
                    }
 					//Now just save first image
    ...
    public function delete_item_key($item_key = null){
        ...
        if(!empty($item_key)){
            try{
                ...
                $items = \Model_Service::find('all', ['select' => ['id'], 'where' => ['item_key' => $item_key]]);

                if(!empty($items)){
                    foreach ($items as $val) {
                        $Ids[] = $val->id;
                        $result = \Model_Service::softDelete($val->id, array('item_status' => 'delete'));
                    }
                }
                \DB::commit_transaction();
            } catch (\Exception $e) {
